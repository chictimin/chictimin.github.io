...
![물론 이렇게 생각하실 수도 있다… 하지만…](/images/img-01.png)
...
![은행에서 은행원은 한 명이지만 대기 인원이 100명이라고 생각하면...](/images/img-02.png)
...
![Style(스타일 트리) → Layout(도형값 계산) → Paint(화면에 그리기) 순서로 진행되는 렌더 파이프라인](/images/browser-style-layout-paint-webdev.gif)
...
![하나의 웹페이지가 실제로 참조하는 다양한 호스트들 — scorecardresearch, quantserve, amazon-adsystem, doubleclick, twitter 등 도메인이 한 페이지 소스에 다 들어 있다](/images/browser-multi-host-source-example.png)
_실제 사이트의 `<head>` 소스. 서드파티 스크립트·폰트·광고·분석 도구까지 합쳐 도메인이 5~10개를 훌쩍 넘는 경우가 드물지 않다._
...
![DNS 조회 → TCP Handshake(SYN/SYN-ACK/ACK) → TLS Negotiation(ClientHello/ServerHello/Certificate) 순서로 이루어지는 연결 수립 과정](/images/browser-dns-tcp-tls-handshake-mdn.png)
_출처: MDN, [브라우저 동작 순서(탐색)](https://developer.mozilla.org/ko/docs/Web/Performance/How_browsers_work#탐색navigation)_
...
![해당 도메인을 처음 조회했을 경우 — DNS Lookup, Initial connection, SSL 과정을 모두 거친다](/images/browser-dns-lookup-first-visit.jpg)
_처음 방문 — DNS Lookup(6.69ms), Initial connection(64.37ms), SSL(60.28ms)까지 전부 거친다._

![동일한 도메인을 다시 조회한 경우 — Stalled 외엔 거의 소요 시간이 없다](/images/browser-dns-lookup-cached-visit.jpg)
_같은 도메인 재방문 — DNS 조회도 연결 수립도 생략되고 Stalled 0.72ms가 전부다._
...
![Network 탭에서 대다수의 사이트가 document 데이터부터 불러온다](/images/browser-network-tab-document-first.png)
...
![HTML 문서에 종속되어 순차적으로 요청되는 자산들의 체인](/images/browser-request-initiator-chain.png)
...
![Send Request와 Receive Response가 찍힌 실제 타임라인](/images/browser-send-request-receive-response.jpeg)
