...
그래서 별도 세션을 하나 열어서, 어제 세션 로그(JSONL 원본)를 직접 파싱해서 실측치를 뽑아봤다. 이 글은 그 결과를 정리한 것 — 숫자로 확인한 병목과, 그래서 앞으로 어떻게 쓸지에 대한 회고다.
...
세션 로그를 실제로 까보고 나서 개선 방안을 우선순위대로 정리했다. 그대로 옮겨두면 이렇다.
