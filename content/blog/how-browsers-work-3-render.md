...
![레이아웃 변경 데모](/images/browser-layout-shift-jank-webdev.gif "예상치 못한 레이아웃 변경이 반복될 때 화면이 튀는 데모 — 출처: web.dev (2022)")
...
![FCP 타임라인](/images/browser-fcp-timeline-webdev.png "구글 모바일 검색 결과 페이지 로딩 타임라인 — 두 번째 프레임에서 FCP가 찍히지만 아직 화면 일부만 보인다 — 출처: web.dev (2019)")
...
![스태킹 콘텍스트 예시](/images/browser-stacking-context-mdn.png "position과 z-index가 겹쳐 정의된 DIV 6개가 스태킹 콘텍스트에 따라 쌓이는 순서 — 출처: MDN")
...
![Layerize/Commit 타임라인](/images/browser-layerize-commit-timeline.png "Layerize와 Commit이 순서대로 진행되는 실제 타임라인")
![Layerize 이벤트 로그](/images/browser-layerize-commit-eventlog.png "Layerize 구간에서 여러 Paint가 쌓인 뒤 Commit으로 이어지는 이벤트 로그 실측 — Chrome DevTools Performance 패널")
