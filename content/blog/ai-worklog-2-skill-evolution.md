...
V1 — 틀린 방향
```
[vault 안의 raw 메모] → AI 요약 → [정식 Research 노트 + 인덱스 갱신]
                    (내가 원한 건 반대 방향이었다)
```

V2 — 방향은 맞지만 비용 구조가 안 맞음
```
[AI 세션 기록] → Claude 헤드리스(유료) → [원자 노트 작성 + 볼트 조회 + 중복검색 + 인덱스 작업]
                  → API 호출당 과금, 매일 밤 돌리기엔 부담
                  → 저장보다 확인 절차에 더 많은 토큰 소모
```

V3 — 캡처와 정리 분리 (Inbox 패턴)
```
[AI 세션 기록] → OpenCode 헤드리스(무료) → 빠른 캡처(모드 1) → inbox.md에 한 줄 추가 (저비용, 매일)
                                                              ↓ 사용자가 요청할 때
                                                       배치 승격(모드 2) → Research 정식 노트 (고비용, 가끔)
```
